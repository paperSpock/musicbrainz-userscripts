...
var MBImport = (function () {
    ...
    function fnBuildFormHTML(parameters) {
        // Build form
        let innerHTML = `<form class="musicbrainz_import" action="//musicbrainz.org/release/add" method="post" target="_blank" accept-charset="UTF-8" charset="${document.characterSet}">`;
        parameters.forEach(function (parameter) {
            let value = `${parameter.value}`;
    ...
    // compute HTML of import form
    function fnBuildArtFormHTML(mb_url, img_url, release_url, importer_name = 'Bandcamp', home = 'https://github.com/paperSpock/musicbrainz-userscripts') {
        // Build form
        var actionURL = mb_url.substring(9, 71) + "/add-cover-art";
        var editNoteName = "add-cover-art.edit_note";
        let innerHTML = `<form class="musicbrainz_import musicbrainz_import_add" action="${actionURL}" method="post" target="_blank" accept-charset="UTF-8" charset="${document.characterSet}">`;
        innerHTML += `<input type='hidden' value='Front cover art added from ${release_url} using ${importer_name} import script from ${home}\n${img_url}' name='${editNoteName}'/>`;

        innerHTML +=
            '<button type="submit" title="Add coverart to this release in MusicBrainz (open a new tab)"><span>Add Coverart</span></button>';
        innerHTML += '</form>';
    ...
    function fnMakeEditNote(release_url, importer_name, format, home = 'https://github.com/paperSpock/musicbrainz-userscripts') {
    ...
    return {
        ...
        BuildArtFormHTML: fnBuildArtFormHTML,
